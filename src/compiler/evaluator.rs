         ...
         -> CorvoResult<()> {
            rt.block_on(async {
                ...
                while let Some(delivery) = consumer.next().await {
                    ...
                    // For the block, provide a copy of thread state plus the shared vars + msg_map
                    let mut eval_state = thread_state.clone();
                    for (k, v) in scope_state.get_all() {
                        eval_state.var_set(k, v);
                    }
                    ...
                    let mut evaluator = Evaluator::new();
                    let result = evaluator.execute_block(&body_clone, &mut eval_state);
