...
impl Transpiler {
    ...
    fn transpile_stmt(&mut self, stmt: &Stmt, state_var: &str) -> String {
        ...
        match stmt {
            ...
            Stmt::Loop { body } => {
                let label_id = self.loop_label_counter;
                self.loop_label_counter = self.loop_label_counter.wrapping_add(1);
                code.push_str(&format!(
                    "{}'corvo_loop_{}: loop {{\n",
                    self.indent(),
                    label_id
        ...
        match expr {
            ...
            Expr::ProcedureLiteral { params, body } => {
                ...
                for stmt in body {
                    ...
                    proc_code.push_str("    if state.var_get(\"_terminate_requested\").unwrap_or_else(|_| Value::Boolean(false)).is_truthy() {\n");
                    proc_code.push_str("        state.var_set(\"_terminate_requested\".to_string(), Value::Boolean(false));\n");
                    proc_code.push_str("        return Ok(Value::Null);\n");
                    proc_code.push_str("    }\n");
                }
...
mod tests {
    ...
    /// Issue #20 / Copilot: labeled loop mirrors `while !terminate_requested`, breaks without
    /// clearing the flag mid-body, then clears once after the loop (evaluator parity).
    #[test]
    fn terminate_inside_loop_transpiles_to_flag_and_checked_break() {
        ...
        assert!(
            rust.contains("'corvo_loop_"),
            "expected labeled loop for nested-safe break:\n{rust}"
        );
        assert!(
            rust.contains("break 'corvo_loop_"),
            "loop must break with labeled break when flag is set:\n{rust}"
        );
        assert!(
            rust.contains(".var_set(\"_terminate_requested\".to_string(), Value::Boolean(false))"),
            "expected terminate flag cleared after loop (evaluator parity):\n{rust}"
        );
    }
}
